...
class CustomerController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $payload = app(CustomerListingService::class)->getIndexPayload(
                $this->company->id,
                $this->currencies,
            ...
            if ($request->ajax()) {
                return view('admin.customers.partials.list', array_merge($payload, [
                    'permissions' => $permissions,
                ]));
            }

            return view('admin.v2.customers.index');
        } catch (\Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('admin.v2.customers.create');
        } catch (\Exception $e) {
     ...
     */
    public function store(Request $request, CustomerPersistenceService $persistence)
    {
        try {
            // Verificar autorización
            if (! Gate::allows('customers.create')) {
        ...
        try {
            ...
            if (request()->has('debt_payment_data')) {
                try {
                    ...
                    $customerSales = Sale::where('customer_id', $customer->id)
                        ->where('company_id', $this->company->id)
                        ->select('sale_date', 'total_price')
                        ->get();

                    $customerPayments = DebtPayment::where('customer_id', $customer->id)
                        ->where('company_id', $this->company->id)
            ...
            if (request()->has('customer_details')) {
                try {
                    ...
                    $lastPayment = null;
                    $lastPaymentRecord = DebtPayment::where('customer_id', $customer->id)
                        ->where('company_id', $this->company->id)
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admin.v2.customers.edit', compact('customer'));
        } catch (\Exception $e) {
     ...
     */
    public function update(Request $request, $id, CustomerPersistenceService $persistence)
    {
        try {
            // Verificar autorización
            if (! Gate::allows('customers.edit')) {
            ...
            $customer = Customer::findOrFail($id);
            $customer = $persistence->validateAndUpdate($customer, $request->all());
    ...
    public function destroy($id)
    {
        try {
            ...
            $paymentsCount = DebtPayment::where('customer_id', $customer->id)->count();
            if ($paymentsCount > 0) {
                $totalPaymentsAmount = DebtPayment::where('customer_id', $customer->id)->sum('payment_amount');
                $firstPaymentDate = DebtPayment::where('customer_id', $customer->id)->orderBy('created_at', 'asc')->first()->created_at->format('d/m/Y');
                $lastPaymentDate = DebtPayment::where('customer_id', $customer->id)->orderBy('created_at', 'desc')->first()->created_at->format('d/m/Y');
    ...
    public function debtReport(Request $request)
    {
        try {
            ...
            $salesInfo = Sale::whereIn('customer_id', $customerIdsArr)
                ->where('company_id', $this->company->id)
            ...
            $paymentsInfo = DebtPayment::whereIn('customer_id', $customerIdsArr)
                ->where('company_id', $this->company->id)
            ...
            foreach ($customers as $customer) {
                ...
                if ($shouldInclude && ($dateFrom || $dateTo)) {
                    if (! $debtSinceDate) {
                        ...
                        $debtDate = Carbon::parse($debtSinceDate)->startOfDay();
    ...
    public function debtReportModal(Request $request)
    {
        try {
            ...
            $salesInfo = Sale::whereIn('customer_id', $customerIds)
                ->where('company_id', $this->company->id)
                ->select('customer_id', 'sale_date', 'total_price')
                ->get()
                ->groupBy('customer_id');

            $paymentsInfo = DebtPayment::whereIn('customer_id', $customerIds)
                ->where('company_id', $this->company->id)
                ->select('customer_id', 'payment_amount', 'created_at')
                ->get()
                ->groupBy('customer_id');
            ...
            foreach ($customers as $customer) {
                ...
                $customersData[$customer->id] = [
                    ...
                    'debt_since_date' => $debtSinceDate ? Carbon::parse($debtSinceDate)->format('Y-m-d H:i:s') : null,
                ];

...
class UserController extends Controller
{
    ...
    public function report(Request $request)
    {
        ...
        $pendingCount = $users->whereNull('email_verified_at')->count();
        $withRolesCount = $users->filter(fn ($user) => $user->roles->isNotEmpty())->count();
        $emittedAt = now();
        $filename = 'reporte-usuarios-'.$emittedAt->format('Y-m-d_His').'.pdf';

        $pdf = Pdf::loadView('pdf.users.report', compact(
            'users',
            'company',
            'verifiedCount',
            'pendingCount',

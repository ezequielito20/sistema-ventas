...
if (typeof PERMISSIONS_CREATE_CONFIG === 'undefined') {
    window.PERMISSIONS_CREATE_CONFIG = {
        routes: {
...
// ===== FUNCIONES GLOBALES =====
if (typeof window.permissionsCreate === 'undefined') {
    window.permissionsCreate = {
        ...
        validatePermissionName: function(input) {
            const value = input.value;
            const isValid = PERMISSIONS_CREATE_CONFIG.validation.nameFormat.test(value);
        ...
        handleFormSubmit: function(form) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-spinner loading-spinner mr-2"></i>Creando...';
        ...
        validateForm: function(form) {
            ...
            const guardSelect = form.querySelector('#guard_name');
            let isValid = true;
            ...
            if (!nameInput.value.trim()) {
                ...
                isValid = false;
            } else if (!PERMISSIONS_CREATE_CONFIG.validation.nameFormat.test(nameInput.value)) {
            ...
            // Validar guard
            if (!guardSelect.value) {
                this.showFieldError(guardSelect, 'Debe seleccionar un guard');
                isValid = false;
            } else {
                this.clearFieldError(guardSelect);
            }
...
function setupFormEvents() {
    ...
    const guardSelect = document.querySelector('#guard_name');
    
    if (form) {
        // Evento de envío del formulario
        form.addEventListener('submit', function(e) {
            if (!window.permissionsCreate.handleFormSubmit(this)) {
                e.preventDefault();
            }
        });
    }
    ...
    if (nameInput) {
        ...
        nameInput.addEventListener('input', function() {
            window.permissionsCreate.formatPermissionName(this);
        });
        
        // Evento de blur para validar
        nameInput.addEventListener('blur', function() {
            window.permissionsCreate.validatePermissionName(this);
        });
    }
    
    if (guardSelect) {
        // Evento de cambio para validar guard
        guardSelect.addEventListener('change', function() {
            if (this.value) {
                window.permissionsCreate.clearFieldError(this);
            } else {
                window.permissionsCreate.showFieldError(this, 'Debe seleccionar un guard');
            }
        });
    }
}
...
if (typeof window.initializePermissionsCreate === 'undefined') {
    window.initializePermissionsCreate = initializePermissionsCreate;
}

// ===== INICIALIZAR CUANDO EL DOM ESTÉ LISTO =====
if (!window.permissionsCreateInitialized) {
    document.addEventListener('DOMContentLoaded', function() {
        if (!window.permissionsCreateInitialized) {
            initializePermissionsCreate();
            window.permissionsCreateInitialized = true;
        }
    });
}

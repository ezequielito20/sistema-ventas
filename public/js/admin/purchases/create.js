  ...
  document.addEventListener('alpine:init', () => {
    Alpine.data('purchaseForm', () => ({
      ...
      updateItemDiscount(index) {
        ...
        // NO re-asignamos item.discountValue = numericVal si el valor es válido
        // Esto permite que el usuario mantenga el formato que está escribiendo (ej. 30.5)
      ...
      cancelPurchase() {
        if (this.formChanged) {
          if (confirm('¿Está seguro? Se perderán todos los datos ingresados en esta compra')) this.goBack();
        } else {
          this.goBack();
        }
      },
      ...
      submitForm(event) {
        ...
        Swal.fire({
          ...
          text: "¿Está seguro de que desea registrar esta compra?",
          icon: 'question',
          ...
          if (result.isConfirmed) {
            try {
              ...
              console.log('Enviando formulario...', { actionName, actionValue });

              // Realizar el submit programático
              form.submit();
            } catch (err) {
              console.error('Error crítico en el envío:', err);
              this.showToast('Error al procesar el envío: ' + err.message, 'error');
      ...
      prepareFormData() {
        ...
        this.products.forEach((p, index) => {
          ...
          const subtotal = this.getItemSubtotalWithDiscount(p);
          createInput(`items[${p.id}][subtotal]`, subtotal);
        });
        ...
        // Asegurar que total_price coincida con el final
      ...
      setupFormSubmitListener() {
        // El submit ya lo manejamos programáticamente en submitForm()
      }
    }));
    ...
    Alpine.data('searchModal', () => ({
      ...
      filterProductsInModal() {
        ...
        document.querySelectorAll('#searchProductModal tbody tr').forEach(row => {
          ...
          const name = row.querySelector('td:nth-child(3) .ml-2 div:first-child')?.textContent.toLowerCase() || ''; // Adjusted selector

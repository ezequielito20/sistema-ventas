...
<div class="bg-gradient-to-r from-slate-50 to-gray-100 rounded-xl sm:rounded-3xl p-3 sm:p-6 md:p-8 lg:p-8 mb-4 sm:mb-8 md:mb-10 lg:mb-10 shadow-lg sm:shadow-xl border border-gray-200/50"
    @if ($sectionId) data-section-id="{{ $sectionId }}" @endif>
    <div
        class="flex flex-col md:flex-row md:items-center lg:flex-row lg:items-center justify-between gap-4 sm:gap-6 md:gap-8 lg:gap-8">
        <!-- Title Section -->
        <div class="flex items-center gap-2.5 sm:gap-4 md:gap-6 lg:gap-6 flex-1 min-w-0">
            <div class="relative flex-shrink-0">
                <div
                    class="flex items-center justify-center w-10 h-10 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-20 lg:h-20 bg-gradient-to-br {{ $iconBg }} rounded-lg sm:rounded-2xl md:rounded-3xl lg:rounded-3xl shadow-lg sm:shadow-xl md:shadow-2xl lg:shadow-2xl">
                    <i class="{{ $icon }} text-base sm:text-xl md:text-3xl lg:text-3xl text-white"></i>
                </div>
                @if ($showStatus)
                    <div
                        class="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-8 lg:h-8 bg-{{ $statusColor }}-500 rounded-full flex items-center justify-center">
                        <i class="{{ $statusIcon }} text-white text-xs sm:text-sm"></i>
                    </div>
                @endif
            </div>
            <div class="flex-1 min-w-0">
                <h3 class="text-lg sm:text-2xl md:text-4xl lg:text-4xl font-black text-gray-800 mb-0.5 sm:mb-2">
                    {{ $title }}</h3>
                <p class="text-xs sm:text-base md:text-lg lg:text-lg text-gray-600 font-medium line-clamp-1">
                    {{ $subtitle }}</p>
                @if ($showLastUpdate)
        ...
        <!-- Controls Section -->
        <div
            class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 md:gap-6 lg:gap-6 flex-shrink-0">
            @if ($showDataSelector)
                <!-- Data Selector -->
                <div class="relative w-full sm:w-auto"
                    @if ($sectionId) data-section-id="{{ $sectionId }}" @endif>
                    <x-filter-select name="data-mode-select" placeholder="📊 Arqueo Actual"
                        searchPlaceholder="Buscar opción..." :items="collect([
                            ['id' => 'current', 'name' => '📊 Arqueo Actual', 'icon' => 'fas fa-chart-bar'],
                            ['id' => 'historical', 'name' => '📈 Histórico Completo', 'icon' => 'fas fa-history'],
                        ])
                            ->merge(
                                collect($dataOptions)->map(function ($option) {
                                    return [
                                        'id' => $option['value'],
                                        'name' => '📋 ' . $option['text'],
                            ...
                            ->merge(
                                collect($cashCounts)->map(function ($cashCount) {
                                    return [
                                        'id' => 'cash_' . $cashCount['id'],
                                        'name' =>
                                            $cashCount['opening_date_formatted'] .
                                            ' - ' .
                                            $cashCount['closing_date_formatted'],
                                        'icon' => 'fas fa-cash-register',
                                        'cashCountId' => $cashCount['id'],
                                    ];
                                }),
                            )" itemKey="id" itemText="name"
                        itemIcon="icon" allItemsText="📊 Arqueo Actual" allItemsIcon="fas fa-chart-bar"
                        :showAllOption="false" :searchable="false" noResultsText="No hay opciones disponibles"
                        containerClass="data-mode-filter-container" :onChange="'window.sectionHeader.onDataModeChange'" selectedValue="current"
            ...
            @if ($actionButton)
                ...
                <a href="{{ $actionButtonUrl }}"
                    ...
                    <span
                        class="sm:hidden">{{ strlen($actionButtonText) > 10 ? Str::limit($actionButtonText, 8) : $actionButtonText }}</span>
                </a>
            @endif
            ...
            @if ($refreshButton)
                <!-- Refresh Button -->
                <button
                    class="bg-gradient-to-r {{ $iconBg }} text-white px-3 sm:px-4 md:px-8 lg:px-8 py-2 sm:py-3 md:py-4 lg:py-4 rounded-xl sm:rounded-2xl font-bold hover:shadow-lg sm:hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 shadow-md sm:shadow-lg text-xs sm:text-sm md:text-base lg:text-base">
                    <i class="{{ $refreshButtonIcon }} text-sm sm:text-base md:text-lg lg:text-lg"></i>
                    <span class="hidden sm:inline">{{ $refreshButtonText }}</span>
                    <span
                        class="sm:hidden">{{ strlen($refreshButtonText) > 10 ? Str::limit($refreshButtonText, 8) : $refreshButtonText }}</span>
                </button>
            @endif
            ...
            @if (!$actionButton && !$refreshButton && $showStatus)
                <div
                    class="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold shadow-md sm:shadow-lg bg-gradient-to-r from-{{ $statusColor }}-500 to-{{ $statusColor }}-600 text-white">

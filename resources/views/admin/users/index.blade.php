...
@section('content')
    <div class="space-y-6" x-data="usersApp()">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-2 sm:gap-3 mb-6">
            <!-- Total de Usuarios -->
            <x-dashboard-widget title="Total de Usuarios" value="{{ $users->count() }}" valueType="number"
                icon="fas fa-users" trend="Registrados" trendIcon="fas fa-user-plus" trendColor="text-green-300"
            ...
            <x-dashboard-widget title="Usuarios Verificados"
                value="{{ $users->where('email_verified_at', '!=', null)->count() }}" valueType="number"
                icon="fas fa-user-check" trend="Verificados" trendIcon="fas fa-check-circle" trendColor="text-green-300"
                subtitle="{{ $users->count() > 0 ? round(($users->where('email_verified_at', '!=', null)->count() / $users->count()) * 100, 1) . '% del total' : '0% del total' }}"
                subtitleIcon="fas fa-percentage" gradientFrom="from-green-500" gradientTo="to-emerald-600"
                progressWidth="{{ $users->count() > 0 ? ($users->where('email_verified_at', '!=', null)->count() / $users->count()) * 100 : 0 }}%"
                progressGradientFrom="from-green-400" progressGradientTo="to-emerald-500" />

            <!-- Usuarios Pendientes -->
            <x-dashboard-widget title="Pendientes de Verificación"
                value="{{ $users->where('email_verified_at', null)->count() }}" valueType="number" icon="fas fa-user-clock"
                trend="Pendientes" trendIcon="fas fa-clock" trendColor="text-yellow-300"
                subtitle="{{ $users->count() > 0 ? round(($users->where('email_verified_at', null)->count() / $users->count()) * 100, 1) . '% del total' : '0% del total' }}"
            ...
            <!-- Usuarios con Roles -->
            <x-dashboard-widget title="Con Roles Asignados"
                value="{{ $users->filter(function ($user) {return $user->roles->count() > 0;})->count() }}"
                valueType="number" icon="fas fa-user-tie" trend="Asignados" trendIcon="fas fa-user-shield"
        ...
        <div class="desktop-view" x-show="viewMode === 'table'">
            <div class="table-container">
                        ...
                        <div class="table-search-container">
                            <div class="flex items-center space-x-4">
                                <div class="search-input-group">
                                    <input type="text" class="search-input" x-model="searchTerm"
                                        placeholder="Buscar usuarios...">
                                </div>
                                <!-- Toggle de Vista - Solo visible en pantallas medianas y grandes -->
                                <div class="hidden md:flex items-center space-x-2">
                                    <button type="button" class="view-mode-btn"
                                        :class="{ 'active': viewMode === 'table' }" @click="changeViewMode('table')">
                                        <i class="fas fa-table mr-2"></i>
                                    </button>
                                    <button type="button" class="view-mode-btn"
                                        :class="{ 'active': viewMode === 'cards' }" @click="changeViewMode('cards')">
                ...
                @include('partials.smart-pagination', ['items' => $users, 'label' => 'usuarios'])
            </div>
        </div>
        ...
        <div class="desktop-view" x-show="viewMode === 'cards'">
            <div class="search-container search-container-right">
                <div class="flex items-center space-x-4">
                    ...
                    <!-- Toggle de Vista - Solo visible en pantallas medianas y grandes -->
                    <div class="hidden md:flex items-center space-x-2">
                        <button type="button" class="view-mode-btn" :class="{ 'active': viewMode === 'table' }"
                            @click="changeViewMode('table')">
                            <i class="fas fa-table"></i>
                        </button>
                        <button type="button" class="view-mode-btn" :class="{ 'active': viewMode === 'cards' }"
                    ...
                    <div class="modal-content-grid">
                        ...
                        <div class="modal-content-column">
                            <div class="modal-detail-item">
                                <span class="detail-label">Roles:</span>
                                <div class="detail-value"
                                    x-html="selectedUser.roles_html || '<span class=\'text-muted\'>Sin rol asignado</span>'">
                                </div>
                            </div>
                            <div class="modal-detail-item">

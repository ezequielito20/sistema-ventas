                ...
                <form id="categoryForm" action="{{ route('admin.categories.store') }}" method="POST">
                    ...
                    <div class="form-actions">
                        ...
                        <div class="actions-right">
                            <button type="button" onclick="goBack()" class="btn-modern btn-cancel">
                                <div class="btn-content">
...
<style>
    ...
    * {
    ...
    .field-group:nth-child(2) { animation-delay: 0.4s; }
    .field-group:nth-child(3) { animation-delay: 0.5s; }
    ...
    .input-wrapper:hover, .textarea-wrapper:hover {
    ...
    .textarea-wrapper:focus-within .input-focus-effect {
        transform: scaleX(1);
    }
    ...
    $(document).ready(function() {
        ...
        function initializeFormInteractions() {
            ...
            window.goBack = function() {
                ...
                card.css('transform', 'translateX(-100%)');
                card.css('opacity', '0');
            ...
            $('.modern-input, .modern-textarea').on('focus', function() {
                $(this).closest('.input-wrapper, .textarea-wrapper').addClass('focused');
                $(this).closest('.field-wrapper').find('.label-icon').css('transform', 'scale(1.1)');
            }).on('blur', function() {
                $(this).closest('.input-wrapper, .textarea-wrapper').removeClass('focused');
                $(this).closest('.field-wrapper').find('.label-icon').css('transform', 'scale(1)');
            });

            // Detección de cambios en el formulario
            $('#categoryForm input, #categoryForm textarea').on('input change keyup', function() {
                formChanged = true;
                clearTimeout(typingTimer);
                typingTimer = setTimeout(updatePreview, doneTypingInterval);
                
                // Efecto visual de cambio
                $(this).closest('.input-wrapper, .textarea-wrapper').addClass('changed');
                setTimeout(() => {
                    $(this).closest('.input-wrapper, .textarea-wrapper').removeClass('changed');
                }, 1000);
            });
        }
        ...
        function initializePreviewSystem() {
            ...
            function updatePreview() {
                const name = $('#name').val().trim() || 'Nombre de la categoría';
                const description = $('#description').val().trim() || 'Descripción de la categoría';

                $('#previewName').text(name);
                $('#previewDescription').text(description);

                // Animación de actualización
                $('.category-preview').css('transform', 'scale(0.98)');
                setTimeout(() => {
                    $('.category-preview').css('transform', 'scale(1)');
                }, 150);
            }
        ...
        function initializeFormValidation() {
            ...
            $('#name').on('input', function() {
                ...
                if (value.length === 0) {
                    ...
                    wrapper.removeClass('valid').addClass('invalid');
                    showFieldMessage(container, `Excede el límite por ${value.length - 255} caracteres`, 'error');
                } else {
            ...
            $('#categoryForm').on('submit', function(e) {
                ...
                if (description.length > 500) {
                    e.preventDefault();
                    showValidationError('La descripción no puede exceder los 500 caracteres');
                    $('#description').focus();
                    return false;
                }
        ...
        function initializeAnimations() {
            ...
            setTimeout(() => {
                $('.progress-step:last-child').addClass('active');
            }, 2000);

            // Efectos de hover en botones
            $('.btn-modern').on('mouseenter', function() {
                $(this).find('.btn-content').css('transform', 'translateY(-1px)');
            }).on('mouseleave', function() {
                $(this).find('.btn-content').css('transform', 'translateY(0)');
            });
        ...
        function showFieldMessage(container, message, type) {
            ...
            // Animar entrada
            const messageEl = container.find('.validation-message').last();
            messageEl.css('opacity', '0').css('transform', 'translateY(-10px)');
            setTimeout(() => {
                messageEl.css('opacity', '1').css('transform', 'translateY(0)');
            }, 50);
        }
        ...
        function showLoadingState() {
            const submitBtn = $('#submitCategory');
            const originalContent = submitBtn.find('.btn-content').html();
            
            submitBtn.prop('disabled', true);
            submitBtn.find('.btn-content').html(`
            ...
            if (!$('.loading-spinner-style').length) {
                $('head').append(`
                    <style class="loading-spinner-style">
                        .loading-spinner {
                            width: 16px;
                            height: 16px;
        ...
        function createRippleEffect(button, event) {
            ...
            $(button).find('.btn-ripple').append(ripple);

...
<div
    {{ $attributes->merge(['class' => "group relative overflow-hidden rounded-lg bg-gradient-to-br $gradientFrom $gradientTo text-white shadow-md hover:shadow-lg transition-all duration-300 h-36 sm:h-36 cursor-pointer widget-responsive"]) }}>
    <!-- Animated Background -->
    <div class="absolute inset-0">
        <div
            class="absolute top-0 right-0 w-20 h-20 {{ str_replace('from-', 'bg-', $gradientFrom) }}/30 rounded-full blur-xl">
        </div>
        <div
            class="absolute bottom-0 left-0 w-16 h-16 {{ str_replace('to-', 'bg-', $gradientTo) }}/30 rounded-full blur-lg">
        </div>
    </div>
    ...
    <div class="relative z-10 p-3 sm:p-4 h-full flex flex-col justify-between">
        ...
        <div class="flex items-center justify-between mb-2">
            <div
                class="flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 bg-white/20 backdrop-blur-lg rounded-md flex-shrink-0">
                <i class="{{ $icon }} text-xs"></i>
            </div>
            <div
                class="flex items-center gap-1 bg-white/20 backdrop-blur-lg px-1 py-0.5 sm:px-1.5 rounded-full text-xs font-bold flex-shrink-0">
        ...
        <div class="absolute bottom-0 left-0 right-0 h-0.5 bg-white/10">
            <div class="h-full bg-gradient-to-r {{ $progressGradientFrom }} {{ $progressGradientTo }} rounded-r-full transition-all duration-1000"
